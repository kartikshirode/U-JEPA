...
v2/
  ...
  tests/                  pytest suite (20 passing, 1 network-gated skip)
```
...
pytest                      # 20 pass, 1 skip (network-gated)
...
Every phase must save its resumable state to a path that survives a kernel restart. `persistence.kaggle_working_dir()` resolves to `/kaggle/working` on Kaggle and `./checkpoints` locally; pair with `persistence.kaggle_input_path(slug)` to pick up a prior session's snapshot.
