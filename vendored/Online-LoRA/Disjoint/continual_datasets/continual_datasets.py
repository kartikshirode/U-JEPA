...
class Imagenet_Sketch(torch.utils.data.Dataset):
    def __init__(self, root, train=True, transform=None, target_transform=None, download=False):        
        self.root = os.path.expanduser(root)
        self.transform = transform
        self.target_transform=target_transform
        ...
        self.url = 'https://drive.google.com/open?id=1Mj0i5HBthqH1p_yeXzsg22gZduvgoNeA'
        self.filename = 'ImageNet-Sketch.zip'

        self.fpath = os.path.join(root, 'sketch')
        if not os.path.isfile(self.fpath):
            if not download:
               raise RuntimeError('Dataset not found. You can use download=True to download it')
            else:
        ...
        if not os.path.exists(os.path.join(root, 'sketch')):
            import tarfile
            tar_ref = tarfile.open(os.path.join(root, self.filename), 'r')
            tar_ref.extractall(root)
            tar_ref.close()
