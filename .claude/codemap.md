...
JEPA-based continual-learning research repo. Two generations live side by side.

- `v2/` is the **active** project: a frozen small LLM core (GPT-2-XL, Qwen2.5-1.5B) plus an external editable memory, two intake gates (plausibility via NLI, truth via FEVER-style verification), and a probe that runs after every fact-merge and auto-rolls-back bad ones. The paper's headline is the ROC of bad-merges-caught vs false-aborts.
