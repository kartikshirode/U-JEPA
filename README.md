...
JEPA-based continual-learning research project. The architecture pivoted in June 2026 from "central JEPA brain + sub-agents" to "frozen core + external memory + probe-gated rollback". The active codebase lives under `v2/`. The original work is frozen under `legacy/v1/`.

## Where to start

- **`v2/`**: the active project (code, tests, scripts, plan, research). Start here.
- **`v2/docs/pipeline-1-build-plan.md`**: the current build plan. Phases sized in 9h Kaggle sessions. Headline result is the ROC curve of bad-merges-caught vs false-aborts on good merges.
- **`v2/docs/compass_artifact_*.md`**: research backing the v2 plan (V-JEPA, world models, editing literature, CLS theory, FEVER, model collapse).
