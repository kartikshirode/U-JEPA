...
def get_dataset(dataset, transform_train, transform_val, args,):
    if dataset == 'CIFAR100':
        ...
        dataset_train = Imagenet_Sketch(args.data_path, train=True, download=True, transform=transform_train).data
        dataset_val = Imagenet_Sketch(args.data_path, train=False, download=True, transform=transform_val).data
...
def split_core50_datasets():
    ...
    split_datasets.append([torch.utils.data.ConcatDataset(train_sets), test_dataset])
    return split_datasets, mask
